...
# Write a function that plots clustered locations and overlays them on a
...
# Change coordinate to be floats not objects and remove the non data
df = df[df.Latitude!='..']
...
# In this case we know how to scale the coordinates. Using standardization would be an error 
# becaues we aren't using the full range of the lat/lng coordinates.
...
dbscan = DBSCAN(eps=eps, min_samples=min_samples, metric='euclidean').fit(coords_scaled)

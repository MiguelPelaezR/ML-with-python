...
print(data.DESCR) #description of the Breast Cancer
...
'''
Por convención, un resultado positivo en una prueba de malignidad indica el diagnóstico de una masa como maligna.
 Por lo tanto, una predicción benigna se considera una predicción negativa. El peor escenario posible es una falsa negativa, es decir, 
 cuando la prueba predice incorrectamente que la masa es benigna siendo en realidad maligna.

En el caso del modelo KNN, el número de falsos negativos es 7, mientras que en el modelo SVM es de 2. 
Podemos decir, entonces, que el modelo SVM tiene una mayor sensibilidad de predicción que el modelo KNN.
'''
...
SVM outperformed KNN in terms of precision, recall, and F1-score for both for the individual classes and their overall averages.
 This indicates that SVM is a stronger classifier. Although KNN performed quite well with an accuracy of 94%, SVM has better ability 
 to correctly classify both malignant and beinign cases, with fewer errors. Given that the goal would be to choose the model with better 
 generalization and fewer false negatives, SVM is certainly the preferred classifier.
'''
...
'''
Un modelo ideal tiene precisión similar en entrenamiento y prueba.

SVM muestra consistencia (97.2% vs 97.1%) → no está sobreajustado.

KNN tiene un pequeño gap (95.5% vs 93.6%) → posible sobreajuste.

SVM supera a KNN en precisión y estabilidad.

No se han ajustado los modelos, solo se compararon con hiperparámetros fijos.
'''
